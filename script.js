...
const modalParticleCanvas = document.getElementById('modal-particle-canvas');
const modalParticleCtx = modalParticleCanvas.getContext('2d');
const CATEGORY_PARTICLE_COLORS = {
  "alkali": "#ff6b9d",
  "alkaline": "#ffaa5c",
  "lanthanide": "#c879ff",
  "actinide": "#ff79c0",
...
function resizeCanvasToDisplaySize(canvas, context, width, height) {
...
function makeBackgroundParticle(width, height) {
  return {
    x: Math.random() * width,
...
function initBackgroundParticles() {
  ...
  const count = Math.max(120, Math.min(260, Math.floor((width * height) / 9000)));
  backgroundParticles = Array.from({ length: count }, () => makeBackgroundParticle(width, height));
}

function emitHoverBurst(x, y, cat, count = 12) {
  const color = CATEGORY_PARTICLE_COLORS[cat] || CATEGORY_PARTICLE_COLORS.unknown;
...
function refreshModalParticles(cat) {
  const rect = modalParticleCanvas.getBoundingClientRect();
  const color = CATEGORY_PARTICLE_COLORS[cat] || CATEGORY_PARTICLE_COLORS.transition;
  const centerX = rect.width * 0.5;
  const centerY = rect.height * 0.5;
  modalParticles = Array.from({ length: 64 }, (_, index) => {
    const angle = (Math.PI * 2 * index) / 64;
    const radius = 28 + Math.random() * Math.min(rect.width, rect.height) * 0.32;
    return {
      angle,
      radius,
...
function syncParticleCanvases() {
  resizeCanvasToDisplaySize(particleCanvas, particleCtx, window.innerWidth, window.innerHeight);
  initBackgroundParticles();
  const modalRect = document.getElementById('modal3d').getBoundingClientRect();
  resizeCanvasToDisplaySize(
    modalParticleCanvas,
    modalParticleCtx,
    Math.max(1, Math.round(modalRect.width || 360)),
    Math.max(1, Math.round(modalRect.height || 280))
  );
}
...
function renderParticleEffects() {
  ...
  particleCtx.fillStyle = 'rgba(10, 12, 20, 0.12)';
  particleCtx.fillRect(0, 0, width, height);

  backgroundParticles.forEach(p => {
    const dx = particlePointer.x - p.x;
    const dy = particlePointer.y - p.y;
    const distance = Math.hypot(dx, dy) || 1;
    const pull = particlePointer.active ? Math.max(0, 120 - distance) / 120 : 0;
    const flowX = Math.sin(p.y * 0.003 + performance.now() * 0.0003) * 0.16;
    const flowY = Math.cos(p.x * 0.0024 + performance.now() * 0.00025) * 0.16;
    p.vx = (p.vx + flowX + (dx / distance) * pull * 0.05) * 0.985;
    p.vy = (p.vy + flowY + (dy / distance) * pull * 0.05) * 0.985;
    p.x += p.vx * p.depth;
    p.y += p.vy * p.depth;

    if (p.x < -20 || p.x > width + 20 || p.y < -20 || p.y > height + 20) {
      Object.assign(p, makeBackgroundParticle(width, height));
    }

    particleCtx.beginPath();
    particleCtx.fillStyle = `rgba(110, 210, 255, ${p.alpha})`;
    particleCtx.arc(p.x, p.y, p.size, 0, Math.PI * 2);
    particleCtx.fill();
  });

  hoverBursts = hoverBursts.filter(p => p.life > 0.02);
  hoverBursts.forEach(p => {
    p.vx *= 0.985;
    ...
    p.life -= p.decay;
    particleCtx.beginPath();
    particleCtx.fillStyle = `${hexToRgba(p.color, p.life * 0.75)}`;
    particleCtx.arc(p.x, p.y, p.size * p.life, 0, Math.PI * 2);
    particleCtx.fill();
  });

  if (overlay.classList.contains('open')) {
    const modalRect = modalParticleCanvas.getBoundingClientRect();
    modalParticleCtx.clearRect(0, 0, modalRect.width, modalRect.height);
    modalParticleCtx.fillStyle = 'rgba(8, 12, 22, 0.14)';
    modalParticleCtx.fillRect(0, 0, modalRect.width, modalRect.height);
    modalParticles.forEach(p => {
      p.angle += p.speed * (1 + p.radius / 120);
      const orbitRadius = p.radius + Math.sin(p.angle * 3) * p.wobble;
      const x = p.centerX + Math.cos(p.angle) * orbitRadius;
      const y = p.centerY + Math.sin(p.angle * 1.1) * orbitRadius * 0.55;
      modalParticleCtx.beginPath();
      modalParticleCtx.fillStyle = hexToRgba(p.color, p.alpha);
      modalParticleCtx.arc(x, y, p.size, 0, Math.PI * 2);
      modalParticleCtx.fill();
    });
  } else {
    modalParticleCtx.clearRect(0, 0, modalParticleCanvas.width, modalParticleCanvas.height);
  }

  requestAnimationFrame(renderParticleEffects);
}
...
ELEMENTS.forEach((el, idx) => {
  ...
  tile.addEventListener('mouseenter', e => {
    ...
    emitHoverBurst(rect.left + rect.width / 2, rect.top + rect.height / 2, el.cat, 14);
  });
  ...
  tile.addEventListener('click', () => {
    const rect = tile.getBoundingClientRect();
    emitHoverBurst(rect.left + rect.width / 2, rect.top + rect.height / 2, el.cat, 20);
    openModal(el);
  });
...
function openModal(el) {
  ...
  requestAnimationFrame(() => {
    syncParticleCanvases();
    refreshModalParticles(el.cat);
    buildAtomScene(el);
  });
}
...
function buildAtomScene(el) {
  ...
  resizeCanvasToDisplaySize(modalParticleCanvas, modalParticleCtx, W, H);
  refreshModalParticles(el.cat);
...
window.addEventListener('resize', () => {
  syncParticleCanvases();
  const modalDetail = document.getElementById('modalDetail');
  const activeZ = modalDetail && modalDetail.dataset.z ? parseInt(modalDetail.dataset.z, 10) : null;
  if (activeZ) {
    const activeElement = ELEMENTS[activeZ - 1];
    if (activeElement) refreshModalParticles(activeElement.cat);
  }
});

window.addEventListener('pointermove', e => {
  particlePointer.x = e.clientX;
  particlePointer.y = e.clientY;
